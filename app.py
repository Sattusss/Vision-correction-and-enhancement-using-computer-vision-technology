import streamlit as st
import tensorflow_hub as hub
import cv2
import tensorflow as tf
import pandas as pd
import pyttsx3
import threading
import time

alarm_sound = pyttsx3.init()
voices = alarm_sound.getProperty('voices')
alarm_sound.setProperty('voice', voices[0].id)
alarm_sound.setProperty('rate', 150)

def voice_alarm(alarm_sound: pyttsx3.Engine,text):
    alarm_sound.say(f'{text} detected')
    alarm_sound.runAndWait()
    time.sleep(5)

detector = hub.load("https://tfhub.dev/tensorflow/efficientdet/lite2/detection/1")
labels = pd.read_csv('labels.csv',sep=';',index_col='ID')
labels = labels['OBJECT (2017 REL.)']

cap = cv2.VideoCapture(0)
width = 640
height = 420


st.title("Vision Correction and enhancement by multimodel computer vision technology")
btn = st.button('Open Camera')
st.sidebar.title("Made by:-")
st.sidebar.header("Satyam Tiwari")
st.sidebar.text("A073145021012")
if btn:
    while(True):
        #Capture frame-by-frame
        ret, frame = cap.read()
        
        #Resize to respect the input_shape
        inp = cv2.resize(frame, (width , height ))

        #Convert img to RGB
        rgb = cv2.cvtColor(inp, cv2.COLOR_BGR2RGB)

        #Is optional but i recommend (float convertion and convert img to tensor image)
        rgb_tensor = tf.convert_to_tensor(rgb, dtype=tf.uint8)

        #Add dims to rgb_tensor
        rgb_tensor = tf.expand_dims(rgb_tensor , 0)
        
        boxes, scores, classes, num_detections = detector(rgb_tensor)
        
        pred_labels = classes.numpy().astype('int')[0]
        
        pred_labels = [labels[i] for i in pred_labels]
        pred_boxes = boxes.numpy()[0].astype('int')
        pred_scores = scores.numpy()[0]
        for score, (ymin,xmin,ymax,xmax), label in zip(pred_scores, pred_boxes, pred_labels):
            if score < 0.5:
                continue  
            score_txt = f'{100 * round(score,0)}'
            img_boxes = cv2.rectangle(rgb,(xmin, ymax),(xmax, ymin),(0,255,0),3)      
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(img_boxes,label,(xmin, ymax-10), font, 0.5, (255,0,0), 1, cv2.LINE_AA)
            cv2.putText(img_boxes,score_txt,(xmax, ymax-10), font, 0.5, (255,0,0), 1, cv2.LINE_AA)
            if label != 'person':
                alarm = threading.Thread(target=voice_alarm, args=(alarm_sound,label))
                alarm.start()
            rgb = cv2.cvtColor(img_boxes, cv2.COLOR_RGB2BGR)
        cv2.imshow('object detection',rgb)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

    cap.release()
    alarm_sound.stop()
    cv2.destroyAllWindows()
